...
function PlaitViz({ ads, website, attribution }) {
  ...
  return (
    <svg viewBox={`0 0 ${W + 80} ${H}`} className="w-full max-w-[260px]">
      ...
      <text x={40 + cx + amp + 22} y={H * 0.18 + 4} textAnchor="start" fontSize="9" fontWeight="700" fill={strandA.color} fontFamily="Space Grotesk, sans-serif" letterSpacing="0.08em">
        ADS
      </text>

      {/* Attribution */}
      <line x1={40 + cx - amp + 2} y1={H * 0.5} x2={40 + cx - amp - 18} y2={H * 0.5} stroke={strandB.color} strokeWidth="1" strokeDasharray="3 2" />
      <text x={40 + cx - amp - 22} y={H * 0.5 + 4} textAnchor="end" fontSize="9" fontWeight="700" fill={strandB.color} fontFamily="Space Grotesk, sans-serif" letterSpacing="0.08em">
        ATTRIBUTION
      </text>

      {/* Website */}
      <line x1={40 + cx + amp - 2} y1={H * 0.8} x2={40 + cx + amp + 18} y2={H * 0.8} stroke={strandC.color} strokeWidth="1" strokeDasharray="3 2" />
      <text x={40 + cx + amp + 22} y={H * 0.8 + 4} textAnchor="start" fontSize="9" fontWeight="700" fill={strandC.color} fontFamily="Space Grotesk, sans-serif" letterSpacing="0.08em">
        WEBSITE
      </text>
    </svg>
  );
          ...
          <div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="text-[#0A0A0A] font-bold text-lg mb-2" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
                  {state.title}
                </p>
