    ...
    <section className="py-28 px-6 bg-white border-t border-gray-100">
      <div className="max-w-6xl mx-auto">
        <div className="mb-14 max-w-xl">
          <p className="text-xs font-bold text-[#6E99AE] uppercase tracking-widest mb-3">Packages</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {packages.map((pkg, i) => (
            ...
            >
              <div className="mb-6">
                <p className={`text-xs font-bold uppercase tracking-widest mb-2 ${pkg.highlight ? "text-white/40" : "text-[#6E99AE]"}`}>
                  {pkg.name}
                </p>
              ...
              <ul className="space-y-3 flex-1 mb-8">
                {pkg.items.map((item) => (
                  <li key={item} className={`flex items-start gap-3 text-sm ${pkg.highlight ? "text-white/70" : "text-[#525252]"}`}>
                    <Check className={`w-4 h-4 shrink-0 mt-0.5 ${pkg.highlight ? "text-white" : "text-[#6E99AE]"}`} />
                    {item}
                  </li>
                ))}
              </ul>
