...
export function LiquidMetalButton({ label = "Get Started", onClick, viewMode = "text" }) {
  ...
  const dimensions = useMemo(() => {
    ...
    const w = Math.max(142, Math.ceil((label || "").length * 8.4) + 48);
    ...
    <div className="relative inline-block">
      <div style={{ perspective: "1000px", perspectiveOrigin: "50% 50%" }}>
        ...
        >
          ...
          >
            {viewMode === "icon" && (
              <Sparkles size={16} style={{ color: "#e8e8e8", filter: "drop-shadow(0px 1px 2px rgba(0, 0, 0, 0.5))", transition: "all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)", transform: "scale(1)" }} />
            )}
            {viewMode === "text" && (
              <span
                style={{
